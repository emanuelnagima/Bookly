...
class LivrosController {
  ...
  async updateStatus(req, res) {
    try {
      ...
      const validarStatuses = ['Disponivel', 'Reservado', 'Emprestado', 'Em manutenção', 'Indisponível'];

      if (!validarStatuses.includes(status)) {
      ...
      const updateLivro = await livrosRepository.updateStatus(id, status);
      res.json({ success: true, data: updateLivro, message: 'Status atualizado com sucesso' });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
